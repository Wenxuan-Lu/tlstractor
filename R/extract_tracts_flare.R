...
#'
#' Parses a FLARE VCF and writes ancestry-specific outputs in one or more formats:
#' `gds`, `txt`, `txt.gz`, `vcf`, `vcf.gz`.
#'
#' @param vcf_path Character; input FLARE VCF path (`.vcf` or `.vcf.gz`).
...
#' - Autosomal chromosomes only (1-22, 01-22, `chr1`-`chr22`, `chr01`-`chr22`; case-insensitive).
...
#'
#' @export
extract_tracts_flare <- function(vcf_path,
                                 num_ancs,
                                 output_dir = NULL,
                                 output_formats = "gds",
                                 chunk_size = 1024L) {
  ...
  if (want_gds) {
    ...
    repeat {
      ...
      if (!length(ln)) stop("EOF before VCF header.", call. = FALSE)
      if (startsWith(ln, "#CHROM") || (startsWith(ln, "#") && !startsWith(ln, "##"))) {
        if (endsWith(ln, "\t")) stop("Malformed VCF header: trailing tab.", call. = FALSE)
        hdr <- strsplit(ln, "\t", fixed = TRUE)[[1]]
        if (length(hdr) < 10L) stop("No samples found in VCF header.", call. = FALSE)
  ...
  # Call C++ function to process VCF and write outputs
