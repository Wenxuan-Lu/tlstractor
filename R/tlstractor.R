...
#'   memory usage.
#' @param mac_threshold Integer; ancestry-specific minor-allele count threshold.
#'   SNPs with any ancestry-specific MAC below this threshold are skipped.
#'   Skipped SNPs are returned in the output with `NA` values for inferential
#'   columns. Default is `20L`.
...
#'   moderate speedup with minimal loss of accuracy. Default is `FALSE`.
#'   If no covariates are provided, fast mode is automatically disabled.
#'
...
#' set `sumstats_path` to a header-only tab-delimited file containing a single
#' line with column names: `CHR`, `POS`, `ID`, `REF`, `ALT`, `BETA`, `SE`, and
#' `GDS_ID`.
#'
#' @export
tlstractor <- function(gds_path, sumstats_path, method, cond_local,
    ...
    method <- tolower(method)
        if (!method %in% c("linear", "logistic")) {
            stop("Method must be 'linear' or 'logistic'.", call. = FALSE)
    }
    ...
    if (!dir.exists(out_dir)) stop(sprintf("Output directory does not exist:\n  %s", out_dir), call. = FALSE)
    ...
    if (snp_start <= 0L) stop("Parameter 'snp_start' must be a positive integer.", call. = FALSE)
    if (!is.null(snp_count)) {
        validate_param_type(snp_count, "integer", "snp_count", check_length_one = TRUE)
        snp_count <- as.integer(snp_count)
        if (snp_count <= 0L) stop("Parameter 'snp_count' must be a positive integer.", call. = FALSE)
    }
    validate_param_type(n_cores, "integer", "n_cores", check_length_one = TRUE)
    n_cores <- as.integer(n_cores)
    if (n_cores <= 0L) stop("Parameter 'n_cores' must be a positive integer.", call. = FALSE)
    validate_param_type(chunk_size, "integer", "chunk_size", check_length_one = TRUE)
    chunk_size <- as.integer(chunk_size)
    if (chunk_size <= 0L) stop("Parameter 'chunk_size' must be a positive integer.", call. = FALSE)

    validate_param_type(mac_threshold, "integer", "mac_threshold", check_length_one = TRUE)
    mac_threshold <- as.integer(mac_threshold)
    if (mac_threshold < 0L) stop("Parameter 'mac_threshold' must be non-negative.", call. = FALSE)
    ...
    if (nsamples_total <= 1L) stop("At least two samples need to be present.", call. = FALSE)
    ...
    if (nsnp <= 0L) stop("No SNPs found in GDS file.", call. = FALSE)
    ...
    if (is.null(num_ancs) || is.na(num_ancs) || num_ancs < 2L) {
        stop("At least two ancestries are required.", call. = FALSE)
    }
    ...
    for (k in 0:(num_ancs - 1L)) {
        ...
        if (is.null(dos_dim) || length(dos_dim) != 2L || dos_dim[1] != nsamples_total || dos_dim[2] != nsnp) {
            stop(sprintf("Dosage/anc%d has incorrect dimensions. Expected [%d, %d], got [%s].",
        ...
        if (is.null(hap_dim) || length(hap_dim) != 2L || hap_dim[1] != nsamples_total || hap_dim[2] != nsnp) {
            stop(sprintf("Hapcount/anc%d has incorrect dimensions. Expected [%d, %d], got [%s].",
                       k, nsamples_total, nsnp, paste(hap_dim, collapse = ", ")), call. = FALSE)
        }
    }

    if (snp_start > nsnp) stop("Parameter 'snp_start' exceeds the number of SNPs in the GDS file.", call. = FALSE)
    if (is.null(snp_count)) {
        snp_end <- nsnp
    } else {
    ...
    if (anyDuplicated(pheno_dt$id)) stop("Duplicate sample IDs found in the phenotype file.", call. = FALSE)
    n_orig <- nrow(pheno_dt)
    pheno_dt <- pheno_dt[is.finite(pheno)]
    if (nrow(pheno_dt) == 0L) stop("All phenotype values are NA or Inf.", call. = FALSE)
    ...
    if (pheno_range[1] == pheno_range[2]) stop("Phenotype has zero variance.", call. = FALSE)
    ...
    if (has_covar) {
        ...
        if (anyDuplicated(covar_dt$id)) stop("Duplicate sample IDs found in the covariate file.", call. = FALSE)
        ...
        if (nrow(covar_mat) == 0L) stop("All covariate rows contain NA or Inf values.", call. = FALSE)
        n_removed <- sum(!ok)
    ...
    if (has_weight) {
        ...
        if (anyDuplicated(weight_dt$id)) stop("Duplicate sample IDs found in the weights file.", call.=FALSE)
        if (any(!is.finite(weight_dt$weight)) || any(weight_dt$weight < 0)) {
            stop("Weights contain invalid values. All weights must be non-negative finite numbers.", call.=FALSE)
        }
    }
    ...
    if (nsamples <= 1L) stop("At least two samples need to be present after intersection.", call. = FALSE)
    ...
    if (has_covar) {
        ...
        if (!all(is.finite(covar_mat))) {
            stop("Covariate matrix contains non-finite values after standardization.", call. = FALSE)
        }
    ...
    if (nsamples < min_samples_required) {
        stop(sprintf("Not enough samples after filtering to fit the model. Need at least %d samples, but got %d.", min_samples_required, nsamples), call. = FALSE)
    }
    if (has_weight) {
        positive_weight <- weights_vec > 0
        n_pos_weight <- sum(positive_weight)
        if (n_pos_weight < min_samples_required) {
            stop(sprintf("Not enough samples with positive weight to fit the model. Need at least %d samples with positive weight, but got %d.", min_samples_required, n_pos_weight), call. = FALSE)
        }
    }

    if (!has_covar && use_fast_version) {
        use_fast_version <- FALSE
        control$use_fast_version <- FALSE
    ...
    if (method == "linear") {
        ...
        if (!all(is.finite(pheno_vec))) {
            stop("Phenotype vector contains non-finite values after standardization.", call. = FALSE)
        }
        if (has_weight) {
            rm(positive_weight)
        }
        linear_precomp_error <- "Linear precomputation (pheno ~ covariates) failed due to high covariate collinearity."
        if (has_covar) {
        ...
        if (length(pheno_unique) != 2L || !all(pheno_unique %in% c(0, 1))) {
            stop("Logistic method requires phenotype values 0/1, and both values must be present.", call. = FALSE)
        }
        if (has_weight) {
            ...
            if (!pheno_0_weight || !pheno_1_weight) {
                stop("Logistic method requires both 0 and 1 among samples with positive weight.", call. = FALSE)
            }
            rm(positive_weight, pheno_0_weight, pheno_1_weight)
        }
        logistic_precomp_error <- "Logistic precomputation (pheno ~ covariates) failed due to near-separation, covariate collinearity, or non-convergence."
    ...
    if (is.null(avail_cores) || is.na(avail_cores) || length(avail_cores) != 1L || avail_cores <= 0L) {
        stop("Failed to detect available CPU cores.", call. = FALSE)
    }
    max_cores <- max(1L, avail_cores - 1L)
    if (n_cores > avail_cores) stop(sprintf("Parameter 'n_cores' (%d) exceeds the maximum usable cores (%d).", n_cores, avail_cores), call. = FALSE)
    ...
    parallel::clusterCall(
        cl,
        function(pkg, gds_path, ids_in_gds, precomp, control, num_ancs, has_covar, method, cond_local, chunk_size, task_file_prefix, task_file_suffix, read_gds_all_then_subset, case_idx) {
...
validate_col_names <- function(file_path, id_col, value_cols, file_context) {
    ...
    stop(sprintf("Invalid ID column name in %s: '%s'. Use a valid R column name (letters, numbers, dots, or underscores; no leading digit).", file_context, id_col), call. = FALSE)
    }

    for (col in value_cols) {
        if (!identical(col, make.names(col))) {
            stop(sprintf("Invalid column name in %s: '%s'. Use a valid R column name (letters, numbers, dots, or underscores; no leading digit).", file_context, col), call. = FALSE)
        }
    }
    ...
    error = function(e) {
        stop(sprintf("Failed to read %s header: %s", file_context, conditionMessage(e)), call. = FALSE)
    }
    )
...
worker_init <- function(gds_path, ids_in_gds, precomp, control, num_ancs,
                        ...
                        read_gds_all_then_subset, case_idx) {
...
run_task <- function(task, worker_env) {
    ...
    case_idx <- worker_env$case_idx

    output_path <- paste0(worker_env$task_file_prefix, task$id, worker_env$task_file_suffix)

    wrote_header <- FALSE
    mac_threshold <- as.double(control$mac_threshold)
    use_fast_version <- isTRUE(control$use_fast_version)
    ...
    af_case_col_names <- paste0("AF_case_", anc_names)
    af_control_col_names <- paste0("AF_control_", anc_names)
    laprop_col_names <- paste0("LAprop_", anc_names)
    laprop_case_col_names <- paste0("LAprop_case_", anc_names)
    laprop_control_col_names <- paste0("LAprop_control_", anc_names)
    ...
    if (method == "logistic") {
        output_colorder <- c("CHROM", "POS", "ID", "REF", "ALT",
            "main_N", "main_N_case", "main_N_control", 
            "AF", af_col_names, af_case_col_names, af_control_col_names, laprop_col_names, laprop_case_col_names, laprop_control_col_names, 
            "has_sumstats", "joint_pval")
    } else {
        output_colorder <- c("CHROM", "POS", "ID", "REF", "ALT", "main_N",
        "AF", af_col_names, laprop_col_names, 
        "has_sumstats", "joint_pval")
    }

    main_n <- length(ids_in_gds)
    main_hap_n <- 2L * main_n
    if (method == "logistic") {
        main_n_case <- length(case_idx)
        main_n_control <- main_n - main_n_case
        case_hap_n <- 2L * main_n_case
        control_hap_n <- 2L * main_n_control
    }
    ...
    while (start <= task$end) {
        ...
        dosage_counts <- matrix(0, nrow = idx_len, ncol = num_ancs)
        hap_counts <- matrix(0, nrow = idx_len, ncol = num_ancs)
        for (k in seq_len(num_ancs)) {
            dosage_counts[, k] <- matrixStats::colSums2(dos_list[[k]])
            hap_counts[, k] <- matrixStats::colSums2(hap_list[[k]])
        }
        total_dosage_counts <- rowSums(dosage_counts)
        if (method == "logistic") {
            case_dosage_counts <- matrix(0, nrow = idx_len, ncol = num_ancs)
            case_hap_counts <- matrix(0, nrow = idx_len, ncol = num_ancs)
            for (k in seq_len(num_ancs)) {
                case_dosage_counts[, k] <- matrixStats::colSums2(dos_list[[k]], rows = case_idx)
                case_hap_counts[, k] <- matrixStats::colSums2(hap_list[[k]], rows = case_idx)
            }
            control_dosage_counts <- dosage_counts - case_dosage_counts
            control_hap_counts <- hap_counts - case_hap_counts
        }
        ...
        if (method == "logistic") {
            data.table::set(chunk_dt, j = "main_N_case", value = rep.int(as.integer(main_n_case), idx_len))
            data.table::set(chunk_dt, j = "main_N_control", value = rep.int(as.integer(main_n_control), idx_len))
        }
        for (k in seq_len(num_ancs)) {
            hapk <- hap_counts[, k]
            ...
            if (method == "logistic") {
                case_hapk <- case_hap_counts[, k]
                case_afk <- case_dosage_counts[, k] / case_hapk
                case_afk[case_hapk <= 0] <- NA_real_
                data.table::set(chunk_dt, j = af_case_col_names[k], value = case_afk)
                data.table::set(chunk_dt, j = laprop_case_col_names[k], value = case_hapk / case_hap_n)

                control_hapk <- control_hap_counts[, k]
                control_afk <- control_dosage_counts[, k] / control_hapk
                control_afk[control_hapk <= 0] <- NA_real_
                data.table::set(chunk_dt, j = af_control_col_names[k], value = control_afk)
                data.table::set(chunk_dt, j = laprop_control_col_names[k], value = control_hapk / control_hap_n)
            }
        }
        data.table::setcolorder(chunk_dt, output_colorder)
        for (col_name in model_col_names) {
            data.table::set(chunk_dt, j = col_name, value = NA_real_)
        }
        data.table::set(chunk_dt, j = fallback_used_col_name, value = NA)

        min_ancestry_mac <- pmin(matrixStats::rowMins(dosage_counts), matrixStats::rowMins(hap_counts - dosage_counts))
        pass_mac_threshold <- min_ancestry_mac > mac_threshold

        idx_withsum <- which(pass_mac_threshold & has_sumstats)
        idx_nosum <- which(pass_mac_threshold & !has_sumstats)

        if (length(idx_withsum) > 0L) {
            if (use_fast_version) {
                res <- if (method == "linear") {
                    fill_chunk_with_sumstats_linear_fast(
                        dos_list = dos_list,
                        hap_list = hap_list,
                        idx = idx_withsum,
                        sumstats_beta = sumstats_beta_chunk[idx_withsum],
                        sumstats_se = sumstats_se_chunk[idx_withsum],
                        precomp = precomp,
                        control = control,
                        has_covar = has_covar,
                        cond_local = cond_local,
            ...
            z_mat <- ensure_matrix_shape(res$z, length(idx_withsum), num_ancs, "res$z")
            res$beta <- ensure_matrix_shape(res$beta, length(idx_withsum), num_ancs, "res$beta")
            res$se <- ensure_matrix_shape(res$se, length(idx_withsum), num_ancs, "res$se")
            pval_mat <- 2.0 * stats::pnorm(abs(z_mat), lower.tail = FALSE)
            if (cond_local) {
                laz_mat <- ensure_matrix_shape(res$laz, length(idx_withsum), num_ancs - 1L, "res$laz")
                res$laeff <- ensure_matrix_shape(res$laeff, length(idx_withsum), num_ancs - 1L, "res$laeff")
                res$lase <- ensure_matrix_shape(res$lase, length(idx_withsum), num_ancs - 1L, "res$lase")
            ...
            data.table::set(chunk_dt, i = idx_withsum, j = "joint_pval", value = joint_pval_vec)
            data.table::set(chunk_dt, i = idx_withsum, j = fallback_used_col_name, value = as.logical(res$used_qr_fallback))
            for (k in seq_len(num_ancs)) {
                data.table::set(chunk_dt, i = idx_withsum, j = beta_col_names[k], value = res$beta[, k])
                data.table::set(chunk_dt, i = idx_withsum, j = se_col_names[k], value = res$se[, k])
                data.table::set(chunk_dt, i = idx_withsum, j = pval_col_names[k], value = pval_mat[, k])
            }
            if (cond_local) {
                for (k in seq_len(num_ancs - 1L)) {
                    data.table::set(chunk_dt, i = idx_withsum, j = laeff_col_names[k], value = res$laeff[, k])
                    data.table::set(chunk_dt, i = idx_withsum, j = lase_col_names[k], value = res$lase[, k])
                    data.table::set(chunk_dt, i = idx_withsum, j = lapval_col_names[k], value = lapval_mat[, k])
                }
            }
        }
