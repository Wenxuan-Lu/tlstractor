...
validate_param_type <- function(value, expected_type, param_name, check_length_one = FALSE) {
  ...
  if (is_missing) {
    stop(sprintf("Parameter '%s' is required and cannot be NULL, NA, or empty.", param_name), call. = FALSE)
  }
  if (check_length_one && length(value) != 1L) {
    stop(sprintf("Parameter '%s' must have length 1, but has length %d.", param_name, length(value)), call. = FALSE)
  }

  if (expected_type %in% c("numeric", "integer")) {
    if (is.numeric(value) && any(!is.finite(value))) {
      stop(sprintf("Parameter '%s' must be finite, but got non-finite values.", param_name), call. = FALSE)
    }
  }
  ...
  if (!type_check) {
    ...
    stop(sprintf("Parameter '%s' must be %s, but got %s.", param_name, expected_type, actual_type), call. = FALSE)
  }
}